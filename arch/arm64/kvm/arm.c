...
#define kvm_hyp_percpu_begin		kvm_nvhe_sym(__per_cpu_start)
#define kvm_hyp_percpu_size		((unsigned long)kvm_nvhe_sym(__per_cpu_end) - \
...
static int init_hyp_mode(void)
{
	...
	/*
	 * .hyp.bss is placed at the begining of the .bss section, so map that
	...
	/* By then we've switched pgds, free the temporary page tables */
	free_hyp_pgds();
